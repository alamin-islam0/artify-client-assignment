...
export default function Slider() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${API_BASE}/arts/featured`, { signal: ctrl.signal });
        if (!res.ok) {
          setItems([]);
          return;
        }
  ...
  const slides = loading || items.length === 0
    ? [
        {
          _id: "placeholder",
  ...
  return (
    <div className="w-full">
      ...
      >
        {slides.map((a) => {
          const bg = a?.image && typeof a.image === "string" && a.image.trim() !== "" ? a.image : PLACEHOLDER;
          const title = a?.title || "Untitled";
          const desc = a?.description || a?.medium || a?.category || "View this artwork on Artify.";
          return (
              ...
              <div className="relative h-full w-full">
                ...
                <div className="absolute inset-0 bg-black/40 flex items-center">
                  <div className="max-w-6xl mx-auto px-4 text-white space-y-5">
                    ...
                    <p className="text-lg text-center md:text-xl max-w-2xl">{desc}</p>
                    ...
                    <div className="flex gap-4 mt-4 flex justify-center">
                      ...
                      {a._id !== "placeholder" ? (
                        <Link to={`/art/${a._id}`} className="btn btn-outline text-white border-white hover:bg-white hover:text-black">
                          View Artwork
                        </Link>
                      ) : (
                        <button className="btn btn-outline text-white border-white" disabled>
                          View Artwork
                        </button>
                      )}
                    </div>
